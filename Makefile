...
lock-python-dependencies-all:
	...
	pixi run --environment $(call get_env_name,3.11) --manifest-path infra/scripts/pixi/pixi.toml \
		"uv pip compile -p 3.11 --no-strip-extras setup.py --extra python-dateutil-build \
		--generate-hashes --output-file sdk/python/requirements/py3.11-python-dateutil-requirements.txt"
	pixi run --environment $(call get_env_name,3.11) --manifest-path infra/scripts/pixi/pixi.toml \
