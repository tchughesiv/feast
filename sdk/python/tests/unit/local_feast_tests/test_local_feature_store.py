...
def test_apply_feature_view(test_feature_store):
    ...
    assert utils.tags_str_to_dict() is None
    ...
    tags_filter = utils.tags_str_to_dict("(' team :matchmaking, tag: two ',)")
    assert tags_filter == tags_dict
    ...
    tags_filter = utils.tags_str_to_dict("('missing:tag,fdsa',fdas)")
    assert tags_filter == tags_dict
